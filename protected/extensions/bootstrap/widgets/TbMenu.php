	...
	*/
	...
	public function init()
	{
		parent::init();
                ...
                $script = "jQuery(document).on('success.pjax', function() {
                jQuery('#" . $this->id . " > .". $this->activeCssClass ."').removeClass('" . $this->activeCssClass . "');
                if(window.location.pathname != '/'){
                    jQuery('#" . $this->id . " > li:has(a[href^=\"' + window.location.pathname + '\"])').addClass('" . $this->activeCssClass . "');
                }else{
                    jQuery('li a').addClass('" . $this->activeCssClass . "');
                }
                });";
                Yii::app()->clientScript->registerScript($this->id . '_menu', $script, CClientScript::POS_BEGIN);
	...
	public function isVertical()
	{
        ...
        public function getId($autoGenerate=true)
        {
            if($this->_id!==null)
                return $this->_id;
            //Необходимо что бы айдишники менюшек, передаваемые в pjax-запросе не пересеклись со статичными на странице
            if (!Yii::app()->request->isAjaxRequest)
                return parent::getId($autoGenerate);
            elseif ($autoGenerate)
                return $this->_id='ajax_' . parent::getId();
        }
