 ...
 */
class ImageKit_GD extends ImageKit
{
	...
	public static function check()
	{
		...
		if (defined('GD_BUNDLED')) {
			...
			ImageKit_GD::$_bundled = GD_BUNDLED;
		} else {
			// Get the version information
			$info = gd_info();

			// Extract the bundled status
			ImageKit_GD::$_bundled = (bool)preg_match('/\bbundled\b/i', $info['GD Version']);
		}
		...
		if (!version_compare($version, '2.0.1', '>=')) {
			throw new CException('ImageKit_GD requires GD version 2.0.1 or greater, you have ' . $version);
		}

		return ImageKit_GD::$_checked = TRUE;
	}

	// Temporary image resource
	protected $_image;

	// Function name to open ImageKit
	protected $_create_function;

	/**
	 * Runs [ImageKit_GD::check] and loads the image.
	 *
	 ...
	 *
	 * @param   ImageKit    $image     watermarking ImageKit
	 * @param   integer  $offset_x  offset from the left
	 * @param   integer  $offset_y  offset from the top
