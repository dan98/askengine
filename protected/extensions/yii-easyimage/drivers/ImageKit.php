<?php
/**
 * ImageKit manipulation support. Allows images to be resized, cropped, etc.
 *
 * @package    Kohana/ImageKit
 * @category   Base
 * @author     Kohana Team
...
Yii::import('easyimage.ImageKit_GD', true);
Yii::import('easyimage.Imagick', true);
class ImageKit
{
	 ...
	 *
	 *     $image = ImageKit::factory('upload/test.jpg');
	 *
	 * @param   string   $file    image file path
	 * @param   string   $driver  driver type: GD, ImageKitMagick, etc
	 * @return  ImageKit
	...
	public static function factory($file, $driver = NULL)
	{
		if ($driver === NULL) {
			...
			$driver = ImageKit::$default_driver;
		}

		// Set the class name
		$class = 'ImageKit_' . $driver;
	 ...
	 *     $image->resize(200, 200, ImageKit::INVERSE);
	 *
	 ...
	 *     $image->resize(200, 500, ImageKit::NONE);
	 *
	 * @param   integer  $width   new width
	 * @param   integer  $height  new height
	 * @param   integer  $master  master dimension
	 * @return  $this
	 * @uses    ImageKit::_do_resize
	 */
	public function resize($width = NULL, $height = NULL, $master = NULL)
	{
		if ($master === NULL) {
			// Choose the master dimension automatically
			$master = ImageKit::AUTO;
		} // ImageKit::WIDTH and ImageKit::HEIGHT deprecated. You can use it in old projects,
		// but in new you must pass empty value for non-master dimension
		elseif ($master == ImageKit::WIDTH AND !empty($width)) {
			$master = ImageKit::AUTO;

			// Set empty height for backward compatibility
			$height = NULL;
		} elseif ($master == ImageKit::HEIGHT AND !empty($height)) {
			$master = ImageKit::AUTO;
		...
		if (empty($width)) {
			if ($master === ImageKit::NONE) {
				// Use the current width
				$width = $this->width;
			} else {
				// If width not set, master will be height
				$master = ImageKit::HEIGHT;
			}
		}
		...
		switch ($master) {
			case ImageKit::AUTO:
				...
				$master = ($this->width / $width) > ($this->height / $height) ? ImageKit::WIDTH : ImageKit::HEIGHT;
				break;
			case ImageKit::INVERSE:
				// Choose direction with the minimum reduction ratio
				$master = ($this->width / $width) > ($this->height / $height) ? ImageKit::HEIGHT : ImageKit::WIDTH;
				break;
		}
		...
		switch ($master) {
			case ImageKit::WIDTH:
				...
				break;
			case ImageKit::HEIGHT:
				// Recalculate the width based on the height proportions
				$width = $this->width * $height / $this->height;
				break;
			case ImageKit::PRECISE:
	 ...
	 *     $image->flip(ImageKit::HORIZONTAL);
	 *
	 *     // Flip the image from left to right
	 *     $image->flip(ImageKit::VERTICAL);
	 *
	 * @param   integer  $direction  direction: ImageKit::HORIZONTAL, ImageKit::VERTICAL
	 * @return  $this
	 * @uses    ImageKit::_do_flip
	 */
	public function flip($direction)
	{
		if ($direction !== ImageKit::HORIZONTAL) {
			// Flip vertically
			$direction = ImageKit::VERTICAL;
		}
	 ...
	 *     $mark = ImageKit::factory('upload/watermark.png');
	 *     $image->watermark($mark, TRUE, TRUE);
	 *
	 * @param   ImageKit    $watermark  watermark ImageKit instance
	 * @param   integer  $offset_x   offset from the left
	 * @param   integer  $offset_y   offset from the top
	 * @param   integer  $opacity    opacity of watermark: 1-100
	 * @return  $this
	 * @uses    ImageKit::_do_watermark
	 */
